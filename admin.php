<?php
	...
	if( $_SESSION['admin_login'] != "yes" ){
		...
		$_SESSION['captcha']["image_src"] =  $_SESSION['captcha']["image_src"]; // "/register/captcha/" . str_replace("aptcha.php", "captcha.php", $_SESSION['captcha']["image_src"] ); 
	}
?>
<html>
<head>
...
<body>
<div style="margin-bottom:20px; padding:10px; font-size:20px; border-bottom: 2px solid #ff790e; line-height: 50px; color: black; background-color: #ffef96; " >
Kriya Admin
</div>
<div style='position:absolute; top:20px; right:20px;' ><a href="?action=logout" >Logout</a></div>
...
<?php	}else if( $_GET['view'] == "report1" && !$_GET['item_id'] ){
		...
		*/
		...
		echo "<table class='ddd' border=1 style=\"border-collapse:collapse;\" width=\"100%\" cellpadding='5'>";
		echo "<thead>";
	...
	<input type='submit' value='Submit' name='ss' style='width:100px !important;'></div>
	<div><input type="hidden" name="action" value="admin_login"></div>
</form>
<?php } ?>
